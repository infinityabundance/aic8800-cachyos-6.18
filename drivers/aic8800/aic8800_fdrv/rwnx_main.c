#define timer_delete_sync timer_delete_sync
#define timer_delete timer_delete
...
static void rwnx_csa_finish(struct work_struct *ws)
{
    ...
    *pos++ = 0;
    ...
    // 5.19.2 - 6.2 requires 3 arguments (using a different flag structure)
    cfg80211_ch_switch_notify(vif->ndev, &csa->chandef, 0);
#else
...
/* --- Safe Bridge Stubs for Kernel 6.18 --- */
...
/* The previous crash happened because this returned NULL.
 *  We now allocate actual memory so the driver has a place to write data. */
void *aicwf_prealloc_txq_alloc(size_t size) {
    void *ptr = kzalloc(size, GFP_KERNEL);
    if (!ptr)
        pr_err("aic8800: Failed to prealloc txq memory\n");
    return ptr;
}
